...
/**
 * Pre-computed CRC-16 (Modbus) lookup table — polynomial 0xA001 (reversed
 * 0x8005), initial value 0xFFFF, no final XOR, byte-reflected.
 *
 * Held as a `Uint16Array` so V8 stores it as a packed typed-array (one
 * branchless 16-bit load per byte) instead of a boxed JS number array.
 * The array is exported because the framing FSM and codec micro-benchmarks
 * inline the table directly into their hot loops and must observe the exact
 * same contents.
 *
 * @note Hot Path: Strictly Inline. Do not refactor into sub-routines.
 */
export const CRC_TABLE = new Uint16Array([
 ...
 * @note Hot Path: Strictly Inline. Do not refactor into sub-routines.
 */
export function crcFixed(data: Uint8Array, start: number, end: number): number {
  let crc = 0xffff;
...
/**
 * Compute CRC-16 (Modbus) across two non-contiguous byte ranges as if they
 * were concatenated.
 *
 * Used by the RTU framing FSM when a frame straddles the residual buffer and
 * the freshly-arrived chunk — concatenating them physically would force a
 * Buffer allocation per byte received, so the dual-segment form sweeps both
 * ranges with the same accumulator and skips the copy entirely.
 *
 * @param head First buffer segment.
 * @param headOff Inclusive byte offset into `head`.
 * @param headLen Number of bytes to consume from `head` starting at `headOff`.
 * @param tail Second buffer segment, logically concatenated after `head`.
 * @param tailOff Inclusive byte offset into `tail`.
 * @param tailLen Number of bytes to consume from `tail` starting at `tailOff`.
 * @returns 16-bit CRC value in `[0, 0xFFFF]` — wire encoding is little-endian.
 *
 * @note Hot Path: Strictly Inline. Do not refactor into sub-routines.
 */
export function crcDual(head: Uint8Array, headOff: number, headLen: number, tail: Uint8Array, tailOff: number, tailLen: number): number {
  let crc = 0xffff;
